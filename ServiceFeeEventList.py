...
def ServiceFee(path):
    ...
    EventList = 'ServiceFeeEventList'
    df_ServiceFeeEventList = pd.DataFrame()
        ...
        for file in files:
            ...
            with open(json_file, 'r') as fp:
                ...
                for record in data:
                    ...
                    order_basic_dict = {}
                    # 手動增加日期一列
                    order_basic_dict['Date'] = file.split('_')[4].strip('.json')
                    if jsonpath(record, '$..AmazonOrderId') is not False:
                        order_basic_dict['OrderId'] = record['AmazonOrderId']
                    else: order_basic_dict['OrderId'] = None
                    ...
                    name = jsonpath(record, '$..FeeType')
                    amount = jsonpath(record, '$..CurrencyAmount')
                    record_name_amount_dicts = dict(zip(name, amount))
                    ...
                    # 將order_dict輸出到csv
                    df_ServiceFeeEventList= df_ServiceFeeEventList.append(order_dict, ignore_index=True)
                    print('===========================一條record輸出完畢=========================')
        return df_ServiceFeeEventList