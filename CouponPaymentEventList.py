...
def CouponPayment(path):

    EventList = 'CouponPaymentEventList'
    df_CouponPaymentEventList = pd.DataFrame()
        ...
        for file in files:
            ...
            with open(json_file, 'r') as fp:
                ...
                for record in data:
                    ...
                    order_basic_dict['OrderId'] = record['CouponId']
                    order_basic_dict['Quantity'] = None
                    order_basic_dict['Currency'] = jsonpath(record, '$..CurrencyCode')[0]
                    order_basic_dict['Type'] = EventList[:-9]
                    order_basic_dict['Description'] = record['SellerCouponDescription']

                    # 以上構造order_basic_dict結束，以下開始構造record_name_amount_dicts:
                    record_name_amount_dicts = {}
                    record_name_amount_dicts['CouponRedemptionFee'] = record['FeeComponent']['FeeAmount']['CurrencyAmount']
                    record_name_amount_dicts['Tax'] = record['ChargeComponent']['ChargeAmount']['CurrencyAmount']
                    ...
                    # 將order_dict輸出到csv
                    df_CouponPaymentEventList = df_CouponPaymentEventList.append(order_dict, ignore_index=True)
                    print('===========================一條record輸出完畢=========================')
        return df_CouponPaymentEventList